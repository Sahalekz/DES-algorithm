...
def decryption():
    ...
    for i in range(iterations_required):
        Binary_cipher_text = conv_to_binary(cipher_text_list)
        decry_initial_permut=mapping(initial_permutation_ptable,Binary_cipher_text)
        Li = decry_initial_permut[:32]
        Ri = decry_initial_permut[32:64]
        for round in range(16):
            expansion_table_result = expansion_table(Ri)
